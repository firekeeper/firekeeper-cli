...
module.exports = (mode) => {
    ...
    progress(request(`${settings.source[0]}/${firekeeper['least']['url']}?v=${Math.random()}`), { throttle: 100 })
        ...
        .pipe(fs.createWriteStream(`${settings[mode].package.path}/${firekeeper['least']['filename']}`))
}
